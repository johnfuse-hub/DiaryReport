...
def fetch_rid_hourly_data():
    ...
    try:
        res = requests.post(RID_ASHX_URL, data=params, verify=False, timeout=15)
        if res.status_code == 200:
            data = res.json()
            rows = data.get("rows", [])
            results = json.loads(json.dumps(STATIONS_METADATA))
            for st in results.values():
                st["history"] = []

            col_map = {
                "C.46": ("wlvalues18", "qvalues18", "wlvalues17", "qvalues17"),
                "C.7A": ("wlvalues21", "qvalues21", "wlvalues20", "qvalues20"),
                "C.47": ("wlvalues24", "qvalues24", "wlvalues23", "qvalues23")
            }

            for r in rows:
                htime = r.get("hourlytime", "")
                for st_id, (wl, q, _, _) in col_map.items():
                    val = r.get(wl)
            ...
            for st_id, st in results.items():
                if st["history"]:
                    ...
                    st["currentLevel"] = 2.08
                    st["flowRate"] = "-"
                    st["latestHour"] = "06.00"

            return results
    except Exception as e:
        print(f"⚠️ เกิดข้อผิดพลาด RID: {e}")

    return json.loads(json.dumps(STATIONS_METADATA))
...
def build_flex_payload(stations):
    ...
    for st_id, st in stations.items():
        stat = evaluate_status(st["currentLevel"], st["warningLevel"], st["criticalLevel"])
        if stat["code"] == "critical": c_crit += 1
        elif stat["code"] == "warning": c_warn += 1

        diff = round(st["bankLevel"] - st["currentLevel"], 2)
        diff_txt = f"⚠️ ล้นตลิ่ง +{abs(diff)} ม." if st["currentLevel"] >= st["bankLevel"] else f"ต่ำกว่าตลิ่ง {diff} ม."
        diff_col = "#dc2626" if st["currentLevel"] >= st["bankLevel"] else "#475569"
        flow_txt = f"ไหล: {st['flowRate']}" if st['flowRate'] == "-" else f"ไหล: {st['flowRate']} cms"
        ...
        boxes.append({
            ...
            "contents": [
                ...
                },
                {
                    ...
                    "layout": "horizontal",
                    "contents": [
                        {"type": "text", "text": f"ระดับน้ำ: {st['currentLevel']:.2f} {st['unit']}", "size": "xs", "color": "#334155", "flex": 3},
                        {"type": "text", "text": flow_txt, "size": "xs", "color": "#64748b", "align": "end", "flex": 2}
                    ]
                },
                {"type": "text", "text": f"เกณฑ์: เฝ้าระวัง {st['warningLevel']} / วิกฤต {st['criticalLevel']} (ตลิ่ง {st['bankLevel']:.2f})", "size": "xxs", "color": "#64748b"},
                {"type": "text", "text": diff_txt, "size": "xs", "weight": "bold" if st["currentLevel"] >= st["bankLevel"] else "regular", "color": diff_col}
            ]
        })
...
def main():
    ...
    print("🌊 กำลังดึงข้อมูลสดจากกรมชลประทาน...")
